...
class AMModel extends Model
{
    ...
    public function getResumeData($param){
        $rawSql = "
            SELECT 
                (SELECT COUNT(DISTINCT nip) FROM txn_alihmedia WHERE MONTH(tanggal_proses) = MONTH(CURDATE()) AND created_by = ? ) total,
