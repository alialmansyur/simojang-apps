...
class HumasModel extends Model
{
    protected $table = 'txn_kehumasan';
    ...
    {
        $bulan = $params['bulan'] ?? [];
        $builder = $this->db->table('txn_kehumasan')
            ->select('*')
    ...
    public function getSummary(array $bulan = [])
    {
        $builder = $this->db->table('txn_kehumasan')
            ->select('
                ...
                SUM(COALESCE(jumlah_konten, 0)) AS total_konten,
                SUM(COALESCE(jumlah_followers, 0)) AS total_followers,
