...
class PNBPDocTypeSeeder extends Seeder
{
    public function run()
    {
        $db = $this->db;

        if (!$db->tableExists('data_pnbp_doc_types')) {
            return;
        }
        ...
        foreach ($docTypes as $doc) {
            $existing = $db->table('data_pnbp_doc_types')->where('doc_type', $doc['doc_type'])->get()->getRowArray();
            if ($existing) {
                $db->table('data_pnbp_doc_types')->where('doc_type', $doc['doc_type'])->update([
                    'number'       => $doc['number'],
                    'title'        => $doc['title'],
                    ...
                    'color'        => $doc['color'],
                    'bg_light'     => $doc['bg_light'],
                    'icon_svg'     => $doc['icon_svg'],
                    'is_status'    => $doc['is_status'],
                    'updated_at'   => $now,
                ]);
            } else {
                $db->table('data_pnbp_doc_types')->insert($doc);
            }
        }
    }
}
