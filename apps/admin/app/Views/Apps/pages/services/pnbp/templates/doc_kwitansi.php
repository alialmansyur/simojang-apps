...
<body>
    ...
    <?php 
        ...
        if (!empty($personel)) {
            foreach ($personel as $p) {
                $totalNominal += (float) ($p['total_biaya'] ?? 0);
            }
        }
    ?>
    ...
    <div class="kwitansi-box">
        <table style="width: 100%; border-bottom: 1px dashed #000; padding-bottom: 8px; margin-bottom: 8px;">
            <tr>
                <td style="width: 50%;">
                    <div style="font-size: 13pt; font-weight: bold; text-decoration: underline;">KWITANSI / BUKTI PEMBAYARAN</div>
                </td>
                <td style="width: 50%; text-align: right; font-size: 9pt;">
                    Tahun Anggaran : <strong><?= date('Y') ?></strong><br>
                    Nomor Bukti : <strong><?= esc($doc['doc_number'] ?: '.../KWT/' . date('Y')) ?></strong><br>
        ...
        <table class="table-kwitansi">
            ...
            <tr>
                ...
                <td class="text-justify">
                    Biaya perjalanan dinas dan pelaksanaan penugasan <?= esc($doc['title']) ?> pada <?= esc($doc['nama_seleksi'] ?? 'Seleksi CAT') ?> bertempat di <?= esc($doc['nama_tilok'] ?? 'Titik Lokasi') ?> tanggal <?= \App\Services\PNBP\PNBPHelper::formatPeriode($doc['period_start_date'] ?? null, $doc['period_end_date'] ?? null) ?> (Rincian terlampir dalam Daftar Nominatif).
                </td>
            </tr>
        </table>
    </div>
    ...
    <table class="signature-table">
        <tr>
            ...
            <td style="width: 33%; text-align: center;">
                ...
                <strong><?= esc($signRight['sign_title'] ?? 'Yang Menerima,') ?></strong>
                ...
                <strong><u><?= esc($signRight['nama'] ?? 'Petugas Pelaksana') ?></u></strong><br>
