...
const processingState = (window.ServiceTableUI && ServiceTableUI.createProcessingState)
    ? ServiceTableUI.createProcessingState('Memuat data naskah...')
...
function updateShownSurat() {
    const info = table.page.info();
    $('#surat-data-shown').text(ServiceTableUI.formatNumber((info && info.recordsDisplay) || 0));
}

function loadSummarySurat() {
    $.ajax({
        url: AppConfig.initGlobal + 'fetch/summary-surat',
        type: 'POST',
        dataType: 'json',
        data: { bulan: selectedBulan },
        success: function (response) {
            const s = response?.summary || {};
            $('#surat-total-data').text(ServiceTableUI.formatNumber(s.total_data || 0));
            $('#surat-total-surat').text(ServiceTableUI.formatNumber(s.total_surat || 0));
            $('#surat-total-klasifikasi').text(ServiceTableUI.formatNumber(s.total_klasifikasi || 0));
            $('#surat-bulan-dipilih').text(ServiceTableUI.formatNumber(selectedBulan.length || 0));
            $('#surat-last-update').text(ServiceTableUI.formatDateTime(s.last_update));
        }
    });
}
    ...
    columns: [
        ...
        { data: 'period_end_date', className: 'text-center' },
        {
            ...
            render: function (data) {
                ...
                if (String(data).toLowerCase() === 'surat masuk') badgeClass = 'bg-light-success';
                else if (String(data).toLowerCase() === 'surat keluar') badgeClass = 'bg-light-primary';
        ...
        if (window.ServiceTableUI) {
            ServiceTableUI.setup({
                ...
                cards: [
                    ...
                    { id: 'total-surat', label: 'Total Naskah', value: '0' },
                    { id: 'total-klasifikasi', label: 'Klasifikasi Aktif', value: '0' },
...
table.on('draw.dt', updateShownSurat);
...
$('#dataTable tbody').on('click', '.btn-remove', function () {
    ...
    Swal.fire({
        ...
        $.ajax({
            type: 'POST',
            url: AppConfig.initGlobal + 'kill/data-surat',
            data: { key },
            dataType: 'json',
...
$('#dataTable tbody').on('click', '.btn-update', function () {
    ...
    $('#DataModal').one('shown.bs.modal', function () {
        form.find('[name="jenis"]').val(row.jenis_id || '').trigger('change');
        const $klasifikasi = form.find('[name="klasifikasi"]');
        if (row.klasifikasi_id && row.klasifikasi_nama) {
            const opt = new Option(row.klasifikasi_nama, row.klasifikasi_id, true, true);
            $klasifikasi.append(opt).trigger('change');
        }
    });
});
