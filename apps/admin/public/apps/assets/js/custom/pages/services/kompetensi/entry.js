$(document).ready(function () {
    ...
    const dropdownOptions = {
        metode: ['CACT', 'Pro ASN', 'Integrasi Data']
    };
    ...
    function addRow() {
        ...
        if (lastRow.length) {
            ...
            refMetode = lastRow.find('select[name="metode[]"]').val();
        }

        const optionsHtml = dropdownOptions.metode.map(opt => `<option value="${opt}" ${opt === refMetode ? 'selected' : ''}>${opt}</option>`).join('');
        ...
        const newRow = $(`
            <tr>
                ...
                <td>
                    <select name="metode[]" class="form-select select2" required>
    ...
    $('#form-usulan').on('submit', function (e) {
        ...
        swlwaitProsessing();
        $.ajax({
            url: AppConfig.initGlobal + 'apps-kompetensi/store-data',
            method: 'POST',
            data: $(this).serialize(),
            dataType: 'json',
    ...
    $('#form-import').on('submit', function (e) {
        ...
        $.ajax({
            url: AppConfig.initGlobal + 'apps-kompetensi/import-data',
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
