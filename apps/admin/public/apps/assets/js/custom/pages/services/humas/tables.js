...
const processingState = (window.ServiceTableUI && ServiceTableUI.createProcessingState)
    ? ServiceTableUI.createProcessingState('Memuat data humas...')
...
function updateShownHumas() {
    const info = table.page.info();
    $('#humas-data-shown').text(ServiceTableUI.formatNumber((info && info.recordsDisplay) || 0));
}

function loadSummaryHumas() {
    $.ajax({
        url: AppConfig.initGlobal + 'fetch/summary-humas',
        type: 'POST',
        dataType: 'json',
        data: { bulan: selectedBulan },
        success: function (response) {
            const s = response?.summary || {};
            $('#humas-total-data').text(ServiceTableUI.formatNumber(s.total_data || 0));
            $('#humas-total-konten').text(ServiceTableUI.formatNumber(s.total_konten || 0));
            $('#humas-total-followers').text(ServiceTableUI.formatNumber(s.total_followers || 0));
            $('#humas-bulan-dipilih').text(ServiceTableUI.formatNumber(selectedBulan.length || 0));
            $('#humas-last-update').text(ServiceTableUI.formatDateTime(s.last_update));
        }
    });
}
    ...
    columns: [
        ...
        { data: 'period_end_date', className: 'text-center' },
        {
            ...
            render: function (data, type) {
                ...
                if (!data) return '-';
                const map = {
                    instagram: { cls: 'bg-light-danger text-danger', label: 'Instagram' },
                    facebook: { cls: 'bg-light-primary text-primary', label: 'Facebook' },
                    twitter: { cls: 'bg-light-dark text-dark', label: 'X / Twitter' },
                    tiktok: { cls: 'bg-light-secondary text-secondary', label: 'TikTok' },
                    youtube: { cls: 'bg-light-danger text-danger', label: 'YouTube' },
                    linkedin: { cls: 'bg-light-info text-info', label: 'LinkedIn' },
                    website: { cls: 'bg-light-success text-success', label: 'Website' },
                    lainnya: { cls: 'bg-light-secondary text-secondary', label: 'Lainnya' }
                };
                const key = String(data).toLowerCase();
                const item = map[key] || { cls: 'bg-light-secondary text-secondary', label: data };
                return `<span class="badge ${item.cls}">${item.label}</span>`;
            }
        },
        { data: 'jumlah_konten', className: 'text-center', render: $.fn.dataTable.render.number(',', '.', 0) },
        { data: 'jumlah_followers', className: 'text-center', render: $.fn.dataTable.render.number(',', '.', 0) },
        ...
        if (window.ServiceTableUI) {
            ServiceTableUI.setup({
                ...
                cards: [
                    ...
                    { id: 'total-konten', label: 'Total Konten', value: '0' },
                    { id: 'total-followers', label: 'Total Followers', value: '0' },
...
table.on('draw.dt', updateShownHumas);
...
$('#dataTable tbody').on('click', '.btn-remove', function () {
    ...
    Swal.fire({
        ...
        $.ajax({
            type: 'POST',
            url: AppConfig.initGlobal + 'kill/data-humas',
            data: { key },
            dataType: 'json',
            success: function (response) {
                if (!response) return;
...
$('#dataTable tbody').on('click', '.btn-update', function () {
    ...
    $('#DataModal').one('shown.bs.modal', function () {
        form.find('[name="media"]').val((row.kanal || '').toLowerCase().trim()).trigger('change');

        const setNumber = (name, value) => {
            const raw = String(value || 0).replace(/\D/g, '');
            form.find(`[name="${name}"]`).val(raw.replace(/\B(?=(\d{3})+(?!\d))/g, ','));
        };

        setNumber('contens', row.jumlah_konten);
        setNumber('followers', row.jumlah_followers);
        setNumber('viewers', row.jumlah_viewers);
    });
});
